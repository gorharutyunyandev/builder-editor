...
import { ELEMENT_ID_ATTRIBUTE, PebRendererOptions } from '../../renderer.types';
import {
  GetParentComponentFunction,
  RENDERER_GET_PARENT_FUNCTION,
...
export abstract class PebAbstractElement implements OnInit, OnDestroy {
  ...
  @ViewChildren(PebRendererChildrenSlot)
  childSlots: QueryList<PebRendererChildrenSlot>;

  constructor(
    @Optional() @Inject(RENDERER_GET_PARENT_FUNCTION) private getParent: GetParentComponentFunction,
    ...
    Object
      ...
      .forEach(([name, element]) => {
        ...
        elementsArr.forEach((node) => {
          ...
          Object.keys(styleChanges.deleted).forEach((prop) => {
  ...
  @HostBinding(`attr.${ELEMENT_ID_ATTRIBUTE}`)
  get attrElementId() {
