...
const log = pebCreateLogger('mail-editor:plugins:edit-product-details');

@Injectable()
export class PebEditorMailProductDetailsPlugin
  ...
  afterGlobalInit(): Observable<PebEditorElement> {
    const productPageFocused$ = this.state.selectedElements$.pipe(
      map((selectedIds) => {
        ...
        return element.definition.type === 'mail-product-details' as PebElementType
          ? element
          : false;
      }),
    );
