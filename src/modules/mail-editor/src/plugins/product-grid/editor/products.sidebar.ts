...
import { Font, ProductDimensions } from './products.plugin';
...
@Component({
  selector: 'peb-mail-editor-products-sidebar',
...
export class PebEditorProductsSidebarComponent extends SidebarBasic
  implements OnInit {
  ...
  ngOnInit() {
    ...
    merge(
      ...
      // this.form.get('template').get('rows').valueChanges,
    )
      .pipe(
        ...
        tap((columns) => {
          const {
            ...
            productTemplateRows,
          } = this.productUtils.getColumnsAndRows(this.component);
          ...
          this.component.dimensions.form.patchValue({
            width: Math.max(
              ...
              this.component.productDimensions.form.value.width * columns + ((columns - 1) * 15),
            ),
            height:
  ...
  private getDirtyValues(form: FormGroup | AbstractControl = this.form) {
    return Object.keys((form as FormGroup).controls).reduce(
      (acc, key) => {
