...
export class PebEditorMailToolbarComponent implements PebEditorAbstractToolbar {
  ...
  openView(element: HTMLElement) {
    ...
    overlay.pipe(
      first(),
      filter(Boolean),
      tap((selectedView: EditorSidebarTypes | MailEditorSidebarTypes) => {
        if (selectedView === MailEditorSidebarTypes.EditMasterPages) {
