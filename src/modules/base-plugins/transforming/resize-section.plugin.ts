...
import { KEY_MAP } from './resize-by-keyboard.plugin';
...
export class PebEditorResizeSectionPlugin implements AfterGlobalInit {
  ...
  private resizeOnKeyboardClick() {
    return this.events.window.keydown$.pipe(
      ...
      filter(filterAnd((evt: KeyboardEvent) => {
        ...
        return evt.shiftKey && Object.keys(KEY_MAP).includes(evt.code) && KEY_MAP[evt.code].axis === Vertical &&
      ...
      map((keyboardEvent: KeyboardEvent): ResizeTick => {
        ...
        const diff = KEY_MAP[keyboardEvent.code].delta;
        return { state, diff };
      }),
  ...
  resizeOnKeyboardHold() {
    ...
    return this.events.window.keydown$.pipe(
      ...
      filter(filterAnd((evt: KeyboardEvent) => {
        const resizingElement =  this.renderer.getElementComponent(this.state.selectedElements[0]);
        return evt.shiftKey
          && Object.keys(KEY_MAP).includes(evt.code)
          && KEY_MAP[evt.code].axis === Vertical
          && resizingElement.definition.type === PebElementType.Section;
      })),
      map((keyboardEvent: KeyboardEvent): ResizeTick => {
        const state = {
        ...
        return { keyboardEvent, state, diff: 0, count: 0 };
      }),
      tap(this.resizeInitHandler),
      switchMap((tick) => {
        ...
        const flow$ = of(tick).pipe(
          ...
          scan((acc, resizeTick) => {
            ...
            resizeTick.diff = KEY_MAP[tick.keyboardEvent.code].delta * acc.count;
            return resizeTick;
          },   tick),
          // map(resizeTick => {
          //   count = count + 1;
