...
import { FillOptions, lineFillOptions } from './line.constant';

type PebElementLineStyles = Partial<PebElementStyles>;
...
export class PebEditorLineSidebarComponent extends AbstractComponent implements OnInit {
  ...
  readonly fillOptions = lineFillOptions;
  ...
  trackFormChanges() {
    merge(
      ...
      this.lineForm.get('opacity').valueChanges.pipe(
        tap((value: string) => this.changeStyle.emit({ opacity: parseFloat(value) })),
      ),
