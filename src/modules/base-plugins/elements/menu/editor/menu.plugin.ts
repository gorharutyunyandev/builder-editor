...
import { AbstractEditElementPlugin, AfterGlobalInit, PebEditorElementMenu } from '@pe/builder-editor';

import { PebEditorMenuSidebarComponent } from './menu.sidebar';

const log = pebCreateLogger('editor:plugins:edit-menu');

export const isArrayEqual = (x, y) => isEmpty(xorWith(x, y, isEqual));

@Injectable()
export class PebEditorMenuPlugin extends AbstractEditElementPlugin<PebEditorMenuSidebarComponent>
  implements AfterGlobalInit {
  static elementTypes = [PebElementType.Menu];

  sidebarComponent = PebEditorMenuSidebarComponent;

  productCategories$ = this.productsApi.getProductsCategories().pipe(
    catchError(() => EMPTY),
    shareReplay(1),
  );
