...
import {
  ...
  getGridStyles,
  PebEditorElement,
  PebEditorElementSection,
  PebEditorSectionLabelsControl,
} from '@pe/builder-editor';

import { PebEditorAddElementPlugin } from '../../../general/add-element.plugin';
  ...
  implements AfterGlobalInit
{
  ...
  constructor(
    injector: Injector,
    private behaviorAddElement: PebEditorAddElementPlugin,
  ) {
    super(injector);
  }
      ...
      section.form.get('default').valueChanges.pipe(
        switchMap((value: boolean) => {
          const newElementDef: PebElementDef = {
            ...
            meta: {
              deletable: (elementCmp.siblings.length === 0 ? false : !value),
            },
          };
