...
import { bufferCount, debounceTime, filter, finalize, map, switchMap, takeUntil, tap } from 'rxjs/operators';
import { cloneDeep, merge as lodashMerge } from 'lodash';

import { pebCreateLogger, PebElementDef, PebElementStyles, PebElementType } from '@pe/builder-core';
import {
...
@Injectable()
export class PebEditorButtonPlugin extends AbstractEditElementPlugin<PebEditorButtonSidebarComponent>
    ...
    return merge(
      this.singleElementOfTypeSelected().pipe(
        switchMap((el: PebEditorElement) => {
          ...
          return merge(
            this.handleAlignmentForm(el, sidebarCmpRef),
  ...
  private doubleClick(): Observable<PebEditorElement> {
    return this.events.contentContainer.dblclick$.pipe(
      map((evt: MouseEvent) => {
        ...
        return this.renderer.getElementComponent(elementCmp.definition.id);
      }),
    ...
    return this.events.window.click$.pipe(
      ...
      switchMap((buffer) => {
        return this.events.window.keydown$.pipe(
          map(event => ({
            event,
            element: this.renderer.getElementComponent(this.state.selectedElements[0]),
          })),
          filter(
            ({ element, event }) =>
  ...
  private get blur$(): Observable<any> {
    return this.state.selectedElements$.pipe(
      map(
        elements => elements?.length && this.renderer.getElementComponent(elements[0]),
      ),
