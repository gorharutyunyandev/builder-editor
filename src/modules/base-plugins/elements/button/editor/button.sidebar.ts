...
import { borderOptions, buttonFillOptions, buttonFontOptions, pageOptions, } from './button.variables';
...
export class PebEditorButtonSidebarComponent extends SidebarBasic implements OnInit, OnDestroy {
      ...
      tap(() => {
        ...
        this.form.patchValue({
          ...
          linkToPage: this.element.data?.action?.type === PebInteractionType.NavigateInternal
            ? this.linkPayload(this.element.data.action.payload).path
            : '',
  ...
  linkPayload(payload: any) {
    return typeof payload === 'string' ? { title: '', path: payload, route: '' } : payload;
  }
