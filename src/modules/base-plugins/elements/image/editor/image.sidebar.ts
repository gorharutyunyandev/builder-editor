...
import { MediaType, PebElementDef, PebElementStyles, PebElementType, PebShopContainer } from '@pe/builder-core';
import {
  AbstractComponent,
  hexToRgba,
  PebEditorElement,
...
export class PebEditorImageSidebarComponent extends AbstractComponent implements OnInit {
  ...
  private fromFormToStyles(formValue: StyleForm): { [key: string]: string } {
    ...
    if (formValue.shadow.enabled) {
      ...
      res.boxShadow = `${x}px ${y}px ${res.shadowBlur}px ${res.shadowColor}`;
    } else {
      res.boxShadow = false;
    }
