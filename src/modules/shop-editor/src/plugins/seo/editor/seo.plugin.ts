...
const log = pebCreateLogger('shop-editor:plugins:seo');

@Injectable()
export class PebEditorShopSeoPlugin implements AfterGlobalInit {
  ...
  afterGlobalInit(): Observable<any> {
    return this.editor.commands$.pipe(
      ...
      switchMap(() => {
        ...
        const sidebarCmpRef = this.editor.insertToSlot(PebEditorShopSeoSidebarComponent, PebEditorSlot.sidebar);
  ...
  trackSidebarChanges(sidebar: PebEditorShopSeoSidebarComponent): Observable<any> {
