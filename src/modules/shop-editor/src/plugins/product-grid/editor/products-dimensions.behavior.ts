...
export class ProductsDimensionsBehavior {
  ...
  handleDimensionsForm(elCmp: PebEditorElement, sidebar: PebEditorProductsSidebarComponent): Observable<any> {
    ...
    return merge(
      productDimensions.form.valueChanges.pipe(
        ...
        tap(([prevValue, value]) => {
          ...
          } = this.productUtils.getGapsFromStyles(elCmp);
          ...
          const nextHeight =
            (value.height * productTemplateRows)
