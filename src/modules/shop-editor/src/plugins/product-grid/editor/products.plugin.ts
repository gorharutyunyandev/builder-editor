...
import { ProductsGapsBehavior } from './products-gaps.behavior';
import { ProductUtils } from './products.utils';
import { ProductsLimitsBehavior } from './products-limits.behavior';
...
export interface PebProductGaps {
  columnGap: number;
  rowGap: number;
}

export interface PebEditorElementPropertyProductGaps extends PebEditorElementProperty<PebProductGaps> {
  activate: () => void;
}

export interface PebEditorElementProduct extends PebEditorElement{
  productGaps?: PebEditorElementPropertyProductGaps;
}


@Injectable()
export class PebEditorShopProductsPlugin extends AbstractEditElementPlugin<PebEditorProductsSidebarComponent>
  ...
  constructor(
    ...
    private productsGapsBehavior: ProductsGapsBehavior,
    private productsLimitsBehavior: ProductsLimitsBehavior,
  ...
  afterGlobalInit(): Observable<any> {
    return this.singleElementOfTypeSelected().pipe(
      switchMap((elCmp) => {
        ...
        this.productsGapsBehavior.initGapsForm(elCmp);
        this.productsLimitsBehavior.initProductLimits(elCmp, sidebar);
        ...
        return merge(
          ...
          this.productsDimensionsBehavior.handleDimensionsForm(elCmp, sidebar),
          this.productsGapsBehavior.handleGapsForm(elCmp, sidebar),
          this.productsLimitsBehavior.handleProductLimits(elCmp, sidebar),
  ...
  private handleProductsDimensionsForm(elCmp: PebEditorElementProduct): Observable<any> {
  ...
  private handleDeleteProducts(
    ...
    return sidebar.removeProductHandler.pipe(
      ...
      switchMap((productsIds) => {
        ...
        const nextStyles = {
          ...elCmp.styles,
          width: Math.ceil(nextColumns * (elCmp.productDimensions.form.value.width
            + elCmp.productGaps.form.value.columnGap)),
          height: Math.ceil(nextRows * (elCmp.productDimensions.form.value.height
            + elCmp.productGaps.form.value.rowGap)),
        ...
        return this.store
          ...
          .pipe(
            ...
            debounceTime(100),
            tap(() => {
              if (elCmp.context?.data) {
                elCmp.context.data = elCmp.context?.data.filter(
  ...
  private productsGridFlow(
    ...
    return sidebar.openProductsGrid.pipe(
      ...
      switchMap(({ elCmp, productsIds }) => {
        ...
        if (!this.productUtils.availableSpace(elCmp, nextStyles)) {
          return EMPTY;
        }
        return this.store
          .updateContext(elCmp.definition.id, {
  ...
  private handleProductFontForm(
    elementCmp: PebEditorElementProduct,
    font: Font,
  ): Observable<any> {
  ...
  protected getDimensionsLimits(elementCmp: PebEditorElementProduct) {
    ...
    } = this.productUtils.getGapsFromStyles(elementCmp);
    ...
    const elementWidthMinDimensions =
      ...
      (productTemplateColumnGap * (productTemplateColumns - 1));

    const elementHeightMinDimensions =
      itemHeight * productTemplateRows +
