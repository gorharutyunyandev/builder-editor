...
export class PebEditorElementAnchorsControl {
  ...
  SECTION_ANCHOR_SIZE = SECTION_ANCHOR_SIZE;
  ...
  getAnchorTypeClass(type: AnchorType): string {
    if ([AnchorType.MiddleLeft, AnchorType.MiddleRight].includes(type)) {
      return 'ew';
    }  if ([AnchorType.TopCenter, AnchorType.BottomCenter].includes(type)) {
      return 'ns';
    }  if ([AnchorType.TopLeft, AnchorType.BottomRight].includes(type)) {
      return 'nwse';
    }  if ([AnchorType.TopRight, AnchorType.BottomLeft].includes(type)) {
      return 'nesw';
    }
  }
    ...
    const points = [
      ...
      },
      {
        ...
        x: (width / 2) - (
          ...
          [PebElementType.Section].includes(this.elementDefinition.type) ? SECTION_ANCHOR_SIZE.width / 2 : 0
        ),
        y: 0,
