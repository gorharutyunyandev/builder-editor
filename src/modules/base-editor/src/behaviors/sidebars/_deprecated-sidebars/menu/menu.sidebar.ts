...
import { PebElementDef, PebElementStyles, pebGenerateId, PebPageType, PebShopRoute, PebShopThemeSnapshot } from '@pe/builder-core';
...
import {
  ...
  spacingOptions,
} from '../../../../shared/_old/text/text-style.constants';
...
// tslint:disable-next-line: component-class-suffix deprecation
export class PebEditorMenuSidebarComponentOld extends SidebarBasic implements OnInit, OnDestroy {
  ...
  readonly routes$: Observable<any> = this.store.snapshot$.pipe(
    ...
    map((snapshot: PebShopThemeSnapshot) =>
      snapshot.shop.routing.reduce(
        (acc, route: PebShopRoute) => {
