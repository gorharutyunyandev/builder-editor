...
import {  onlyOneElementSelected } from '../../../_utils/filters';
import { PebEditorMenuSidebarComponentOld } from './menu.sidebar';

@Injectable({ providedIn: 'any' })
export class PebEditorBehaviorEditMenuOld {
  constructor(
  ...
  init(): Observable<any> {
    const menuFocused$ = this.state.singleSelectedElement$.pipe(
      filter(Boolean),
      map(this.renderer.getElementComponent),
      filter(elCmp => elCmp?.definition.type === PebElementType.Menu),
    );
