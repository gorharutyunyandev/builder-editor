...
export class PebEditorColorPickerComponent extends AbstractComponent implements OnInit, OnChanges {
  @Input() control: FormControl = new FormControl(null);
  @Input() pebColorPicker = null;
  @Input() simple = false;
  @Input() displayDash = true;

  // TODO: implement ControlValueAccessor instead
  colorValue$ = new BehaviorSubject<string>('');

  // TODO: Remove it
  @Input() set initialValue(value) {
    this.pebColorPicker = value;
    this.colorValue$.next(value);
  }
  ...
  palette = COLORS.map(({ name, hex }) => ({ name, hex, value: name }));

  ngOnChanges() {
    this.colorValue$.next(this.pebColorPicker);
  }

  ngOnInit(): void {
    if (this.control?.value) {
      this.pebColorPicker = this.control.value;
      this.colorValue$.next(this.control.value);
    }
