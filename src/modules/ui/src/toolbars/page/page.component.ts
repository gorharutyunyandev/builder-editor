...
import { AbstractComponent } from '@pe/builder-renderer';
...
export class PebPageToolbarComponent extends AbstractComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    const observables: Array<Observable<{field: string, fieldValue: string}>> = FORM_FIELDS.map(field =>
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      ...
      (
        ...
        && changes.page.currentValue.id !== changes.page.previousValue.id
      )
    ) {
      this.form.setValue(
...
function getControl(
  defaultValue: any,
  validators: Array<(ValidatorFn | ValidationErrors)>,
